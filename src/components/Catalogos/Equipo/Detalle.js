...
const Detalle = ({seleccionado, onGuardar, catalogo}) => {
    ...
    const guardarRegistro = () => {
        guardaGenerico('equipo', registro)
            .then(res => {
                cerrarAlert();
                onGuardar(registro);
            })
        ...
        <div className="card border-tabla z-depth-2">
            ...
            <div className="m-2">
                ...
                <div className="d-flex mb-2 mt-1">
                    <span className="py-0 pr-3 mt-1 text-nowrap">{i18next.t('catalogos:tipoEquipo')}:</span>
                    <Select onSelect={e => setRegistro({...registro, tipo_equipo: e})}
                            selected={registro.tipo_equipo}
                            options={catalogo}
                            labelKey="nombre"/>
                </div>
                <div className="d-flex mb-2">
                    <span className="py-0 pr-3">{i18next.t('catalogos:uso')}:&nbsp;</span>
                    <span className="mr-2" onClick={()=>setRegistro({...registro,uso:"M"})}>
                        {registro.uso==="M"?<FaRegCheckSquare size={30}/>:<FaRegSquare size={30}/>}
                        {i18next.t('catalogos:monitoreo')}
                    </span>
                    <span onClick={()=>setRegistro({...registro,uso:"V"})}>
                        {registro.uso==="V"?<FaRegCheckSquare size={30}/>:<FaRegSquare size={30}/>}
                        {i18next.t('catalogos:verificacion')}
                    </span>
                </div>
                <div className="d-flex mb-2 mt-1">
                ...
                <div className="mt-3">
                    <Boton
                        ...
                        posicion={'right'}
                        habilitado={propiedadValida(registro, 'nombre') && can('catalogos.guardar_equipo')}
