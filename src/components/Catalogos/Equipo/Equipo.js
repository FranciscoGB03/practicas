...
const Equipo = () => {
    //|----------------|hooks|-----------------|//
    let peticiones=[{ 'nombre': 'tipo_equipo', relaciones: [],  metodo:'orderBy' }];
...
//|------DatosIniciales------|//
    const cargarRegistros = () => {
        getAllGenerico("equipo",['tipo_equipo']).then(res => {
            cerrarAlert();
            setRegistrosAll(res);
        }).catch(noop());
    };
    const getCatalogo=()=>{
        getCatalogos(peticiones, false).then(res => {
            setCatalogo(res.tipo_equipo.data);
            cerrarAlert();
        });
    }
//|------Render------|//
            ...
            {isLogged() ?
                <div>
                    <div className="container">
                        <Titulo titulo={i18next.t('catalogos:tituloEquipo')} icono={BiDevices}/>
                    </div>
