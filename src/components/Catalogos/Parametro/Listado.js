...
import {AiOutlineCloseCircle,BiCheckCircle} from 'react-icons/all';
                ...
                <div id="div_tabla">
                    <table className=" table table-condensed responsive hover ">
                        ...
                        {(registros || []).map((reg, idx) => <tr key={idx}>
                            <td>{reg.tipo_parametro.nombre}</td>
                            <td>{reg.nombre}</td>
                            <td className="text-center">{reg.ver_en_equipos?<BiCheckCircle color="green"/>:<AiOutlineCloseCircle color="red"/>}</td>
                            <td className="text-center">{reg.ver_en_solicitudes?<BiCheckCircle color="green"/>:<AiOutlineCloseCircle color="red"/>}</td>
    ...
    return (
        ...
        <tr>
            <th className='text-center cursor'
                ...
                {i18next.t('catalogos:tipoParametro')}
            </th>
            <th className='text-center cursor'
            ...
            <th>{i18next.t('catalogos:enEquipos')}</th>
            <th>{i18next.t('catalogos:enSolicitudes')}</th>
            <th></th>
        </tr>
        </thead>
    );
