    ...
    return (
        ...
        <tr>
            <th className='text-center cursor'
                onClick={() => handleSort('tipo_pm.nombre')}>
                <IconosSort key_sort={'tipo_pm.nombre'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:tipoPM')}
            </th>
            <th className='text-center cursor'
...
const TheadBuscador = ({filtro, setFiltro}) => {
    return (
        ...
        <tr>
            <th className='py-1'>
                <input className='form-control form-control-sm' value={filtro.tipo_pm?.nombre || ""}
                       placeholder={i18next.t('catalogos:buscar')}
                       onChange={(e) => setFiltro({...filtro, tipo_pm:{nombre: e.target.value}})}/>
            </th>
            <th className='py-1'>
