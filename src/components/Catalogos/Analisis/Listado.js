    ...
    return (
        <div className="card d-flex mx-4 border-tabla z-depth-2 animated bounceInUp">
            <div className=" card-header d-flex justify-content-between text-center py-0">
                ...
                <div className='d-flex justify-content-end text-titulo-tabla mt-1'>
                ...
                <div id="div_tabla">
                    <table className=" table table-condensed responsive hover ">
                        ...
                        {(registros || []).map((reg, idx) => <tr key={idx}>
                            <td>{reg.area_ca.nombre}</td>
                            <td>{reg.nombre}</td>
                            <td className="text-center">{reg.estatus}</td>
    ...
    return (
        ...
        <tr>
            <th className='text-center cursor'
                onClick={() => handleSort('area_ca.nombre')}>
                <IconosSort key_sort={'area_ca.nombre'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:area')}
            </th>
            <th className='text-center cursor'
...
const TheadBuscador = ({filtro, setFiltro}) => {
    return (
        ...
        <tr>
            <th className='py-1'>
                <input className='form-control form-control-sm' value={filtro.area_ca?.nombre || ""}
                       placeholder={i18next.t('catalogos:buscar')}
                       onChange={(e) => setFiltro({...filtro, area_ca:{nombre: e.target.value}})}/>
            </th>
            <th className='py-1'>
            ...
            <th className='py-1'>
                <input className='form-control form-control-sm' value={filtro.estatus || ""}
                       placeholder={i18next.t('catalogos:buscar')}
