...
import {propiedadesValidas, propiedadValida} from "../../../services/validatorService";
...
const Detalle = ({seleccionado, onGuardar, catalogo}) => {
    ...
    const guardarRegistro = () => {
        guardaGenerico('analisis', registro)
            .then(res => {
                cerrarAlert();
                onGuardar(registro);
            })
    ...
    return (
        <div className="card border-tabla z-depth-2">
            <div className="card-header d-flex justify-content-between py-0">
                ...
                <div className="d-flex justify-content-center text-info text-titulo-tabla mt-2">
            ...
            <div className="m-2">
                ...
                <div className="d-flex mb-2 mt-1">
                    <span className="py-0 pr-3 mt-1 text-nowrap">{i18next.t('catalogos:areaCA')}:</span>
                    <Select onSelect={e => setRegistro({...registro, area_ca: e})}
                            className={`${!registro.area_ca ? 'is-invalid' : ''}`}
                            selected={registro.area_ca}
                            options={catalogo}
                            labelKey="nombre"/>
                </div>
                <div className="d-flex mb-2 mt-1">
                ...
                <div className="mt-3">
                    <Boton
                        ...
                        posicion={'right'}
                        habilitado={propiedadesValidas(registro, ['nombre','area_ca','estatus']) && can('catalogos.guardar_analisis')}
